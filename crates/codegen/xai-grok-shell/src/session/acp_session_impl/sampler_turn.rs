...
impl SessionActor {
    ...
    /// identity in as `frozen`, except `Parked` resubmits which skip every
    /// refresh-driving prepare; the wire bearer then comes from the live
    /// resolver at send time.
    pub(crate) async fn run_turn_via_sampler(
        ...
        // response identity) except parked resubmits, which skip that refresh.
        if park.is_parked() {
            // Parked: do not re-push config; send-time resolver supplies the bearer.
        }
        if !budget.can_wait() {
        ...
        loop {
            match self.submit_turn_request(request.clone()).await {
                ...
                Err(info) => {
                    ...
                    // with (FIFO UpdateConfig → Submit still holds). Parked turns skip it.
                    if !park.is_parked() {
